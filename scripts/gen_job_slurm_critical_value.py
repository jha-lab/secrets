...
sys.path.append("..")
import src
from src.critical_value import add_arguments
...
with open(os.path.join(args.results_path,"job.slurm"), 'w') as f:
	f.write("#!/bin/bash\n")
	f.write("#SBATCH --job-name=cv # create a short name for your job\n")
	f.write("#SBATCH -o %s\n" %out_path)
	f.write("#SBATCH -e %s\n" %err_path)
	...
	f.write("\n")
	f.write("srun -u python ../src/critical_value.py \\\n")	

	for key, value in vars(args).items():
		# ignore the SLURM
		if key in ["num_workers", "mem_per_cpu", "gpus", "time", "module_name", "conda_env"]:
			continue
