...
syn_ctrl_champ.add_arguments(parser)
...
with open(os.path.join(args.results_path,"job.slurm"), 'w') as f:
	...
	# if args.use_della: f.write("#SBATCH --constraint=cascade")
	if args.use_della: f.write("#SBATCH --exclude=della-r4c[1-4]n[1-16],della-r1c[3-4]n[1-16],della-r3c[1-4]n[1-16]")
	...
	f.write("\n")
	f.write("srun -u python ../src/syn_ctrl_champ.py \\\n")	

	for key, value in vars(args).items():
		# ignore the SLURM
		if key in ["num_workers", "mem_per_cpu", "gpus", "time", "module_name", "conda_env", "use_della"]:
			continue
